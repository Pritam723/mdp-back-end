...
def createRealMeterMWH(path,_meterData) :
    ...
    relativeMediaPath = 'fifteenmmdp/media'
    meterFileMainFolder = os.path.join(relativeMediaPath,"meterFile",path) #Later "Validated File" path is added
    
    relativeFilePathCopy = meterFileMainFolder+'/SEMBASE(Real Meter)/'
    relativeFilePath = meterFileMainFolder+'/Real Meter MWH File/'

    if not os.path.exists(relativeFilePathCopy): 
        os.makedirs(relativeFilePathCopy)
    ...
    if(not (_meterData.status is None) and (statusCodes.index(_meterData.status) == 4)) :
        ...
        zipf = zipfile.ZipFile(meterFileMainFolder+'/SEMBASE(Real Meter).zip', 'w', zipfile.ZIP_DEFLATED)
        zipdir(relativeFilePathCopy, zipf)
        zipf.close()
        path0 = meterFileMainFolder+'/SEMBASE(Real Meter).zip'
        nPath = 'meterFile/'+path+'/Real Meter MWH File/SEMBASE(Real Meter).zip'
        local_file = open(path0,"rb")  #Open file from path0 i.e. extracted first path.
        realMeterMWHFileObject = RealMeterMWHFile.objects.create(fileName = os.path.basename(nPath), filePath = nPath, meterFile = _meterData)
        realMeterMWHFileObject.realMeterMWHFile.save(os.path.basename(nPath),  File(local_file))
        realMeterMWHFileObject.save()
